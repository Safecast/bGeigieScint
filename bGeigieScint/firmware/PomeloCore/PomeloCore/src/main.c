...
#define DBL_TAP_MAGIC                                                          \
  0xf01669ef // Randomly selected, adjusted to have first and last bit set
...
typedef enum _UsbState_t { STATE_CMD_IDLE, STATE_CMD_PARAM } CmdState_t;
...
bool uart_available() {
  if ((rxBufTail + 1) % UART_BUFFER_LEN != rxBufHead)
    return true;
  else
    return false;
}
...
uint8_t uart_tx(uint8_t *data, uint8_t length) {
  ...
  do {
    status = usart_serial_write_packet(&app_uart_module, (const uint8_t *)data,
                                       length);
...
void uart_init() {
  ...
  host_uart_config.mux_setting = USART_RX_3_TX_2_XCK_3;
  host_uart_config.pinmux_pad0 = PINMUX_UNUSED;
  host_uart_config.pinmux_pad1 = PINMUX_UNUSED;
  host_uart_config.pinmux_pad2 = PINMUX_PB10D_SERCOM4_PAD2;
  host_uart_config.pinmux_pad3 = PINMUX_PB11D_SERCOM4_PAD3;
  // host_uart_config.baudrate    = 115200;
  host_uart_config.baudrate = 921600;
  host_uart_config.run_in_standby = true;
  stdio_serial_init(&app_uart_module, SERCOM4, &host_uart_config);
  ...
  rxBufHead = 1;
  rxBufTail = 0;
  _sercom_set_handler(4, USART_ISR_VECT);
      ...
      if (gammaCoincidence) {
        if (port_pin_get_input_level(PIN_PA20)) {
          ...
          while (port_pin_get_input_level(PIN_PA20)) {
            ...
            port_pin_set_output_level(PIN_PB11,
...
static void rtc_callback(void) { daq_stop(); }
...
static float hvload_uA() {
  ...
  if ((hvloadTime < 65535) && (hvloadOff != 0)) {
    ...
    // 5.6 MOhm resistor in LT3014 feedback loop steals some current too. But
    // resistor value is loaded from parameter
    if (coreParams.iMeas[2] != 0)
      iOut -= (hvSipmVolts - 1.22) / coreParams.iMeas[2];
    if (iOut < 0)
      iOut = 0;
  } else {
    iOut = 9999;
...
static void read_physics(uint8_t n, uint8_t *data) {
  ...
  while (timeOut > 0) {
    status =
        i2c_master_write_packet_wait_no_stop(&i2c_instance_app, &master_packet);
    if (status == STATUS_OK)
      break;
    delay_ms(10);
    timeOut--;
...
static void write_physics(uint8_t page, uint8_t n, uint8_t *data) {
  ...
  for (i = 0; i < n; i++)
    status = i2c_master_write_byte(&i2c_instance_app, data[i]);
...
static void update_hv_temp(bool force) {
  ...
  temp = read_temp();
  // Maybe need to update with values from datasheet numbers: 0.03633 * dDAC/dV.
  // Comes out to -10.0961 for one board
  // delta = (int16_t)(-5.8784*(temp - tempCompTemp));		// Calculated
  // from Am-241 data
  hvSipmVolts = physicsParams.sipm_v0deg + physicsParams.sipm_vTempComp * temp;
  if (hvSipmVolts > physicsParams.sipm_vMax)
    hvSipmVolts = physicsParams.sipm_vMax;
  if (hvSipmVolts < physicsParams.sipm_vMin)
    hvSipmVolts = physicsParams.sipm_vMin;
...
static void eic_init() {
  ...
  extint_register_callback(synchronizer_callback, 8,
...
static void rtc_init_gamma(void) {
  ...
  rtc_count_register_callback(&rtc_instance_app, rtc_callback,
                              RTC_COUNT_CALLBACK_COMPARE_0);
  rtc_count_enable_callback(&rtc_instance_app, RTC_COUNT_CALLBACK_COMPARE_0);

  rtc_count_register_callback(&rtc_instance_app, rtc_callback_periodic,
                              RTC_COUNT_CALLBACK_PERIODIC_INTERVAL_7);
  rtc_count_enable_callback(&rtc_instance_app,
                            RTC_COUNT_CALLBACK_PERIODIC_INTERVAL_7);
}
...
static int8_t adc_write_constants(uint16_t nRow, uint8_t *data) {
  ...
  sc =
      nvm_erase_row(baseAddress + nRow * NVMCTRL_PAGE_SIZE * NVMCTRL_ROW_PAGES);
  if (sc != STATUS_OK)
    return -1;

  for (i = 0; i < NVMCTRL_ROW_PAGES; i++) {
    sc = nvm_write_buffer(baseAddress +
                              nRow * NVMCTRL_PAGE_SIZE * NVMCTRL_ROW_PAGES +
                              i * NVMCTRL_PAGE_SIZE,
                          &data[i * NVMCTRL_PAGE_SIZE], NVMCTRL_PAGE_SIZE);
    if (sc != STATUS_OK)
      return -2;
  }
  return 0;
...
static int8_t adc_compute_constants(uint32_t *calibSpectrum) {
  uint16_t
      rows[256]; // 256 = NVMCTRL_PAGE_SIZE (64) * NVMCTRL_ROW_PAGES (4) *
                 // number_of_rows_in_my_buffer (2) / bytes_per_uint16_t (2)
  ...
  for (i = 0; i < 1024; i++) {
    // Default values that will stay like this for attractor bins, or for first
    // and last bins
    ...
    if ((i >= FILTER_LEN + 2) && (i <= 1023 - FILTER_LEN - 2)) {
      ...
      if ((double)calibSpectrum[i] - avgVal > 0) {
        // Donor bin, pMove is probability to give current sample away to an
        // acceptor
        pMoveBin[FILTER_LEN] =
            ((double)calibSpectrum[i] - avgVal) * 32768.0 / calibSpectrum[i];

        // Count number of samples above average (that will be transferred from
        // donors to acceptors) in the current filter context
        totalToMove = 0;
        for (j = 0; j < 2 * FILTER_LEN + 1; j++) {
          if ((double)calibSpectrum[p + j] - avgVal > 0)
            totalToMove += (double)calibSpectrum[p + j] - avgVal;
        }

        // pMove is probability that that neighbour receives a sample in the
        // current context
        for (j = 0; j < 2 * FILTER_LEN + 1; j++) {
          if (j == FILTER_LEN)
            continue;
          ...
          if (j == FILTER_LEN + 1)
            pMoveBin[j] = pMoveBin[j - 2];
          else if (j != 0)
            pMoveBin[j] = pMoveBin[j - 1];
          if (avgVal - calibSpectrum[p + j] > 0) {
            // Acceptor bin. Populate pMove as sum
            pMoveBin[j] +=
                (avgVal - (double)calibSpectrum[p + j]) * 32768.0 / totalToMove;
          }
        }
      }
    }
...
static void adc_calib(uint8_t iFace) {
  ...
  for (i = 0; i < 1000; i++) {
    ...
    if (i % 10 == 0)
      pomelo_sprintf(iFace, "{\"type\":\"calib_progress\", \"payload\":%d}\n",
                     i / 10);
  }
...
static void pomelo_off() {
  ...
  if (daq_enabled) {
    ...
    dac_chan_write(
        &dac_instance_app, DAC_CHANNEL_1,
        4095); // Minimize the chance that spurious signals will trigger readout
    hv_disable();
...
static void load_parameters(void) {
  ...
  for (i = 0; i < readBytes / NVMCTRL_PAGE_SIZE; i++) {
    ...
    nvm_read_buffer(NVM_RWW + i * NVMCTRL_PAGE_SIZE,
                    (uint8_t *)(&coreParams) + i * NVMCTRL_PAGE_SIZE,
                    NVMCTRL_PAGE_SIZE);
  }
  ...
  if (coreParams.version == 0xFF) {
    ...
    coreParams.sys_outputs =
        0; // There is a variable that this has to be synched up with
    coreParams.sys_coincidence =
        false; // There is a variable that this has to be synched up with
...
static int8_t save_parameters(void) {
  ...
  for (i = 0; i < writeBytes / (NVMCTRL_PAGE_SIZE * NVMCTRL_ROW_PAGES) + 1;
       i++) {
  ...
  for (i = 0; i < writeBytes / NVMCTRL_PAGE_SIZE; i++) {
    ...
    sc = nvm_write_buffer(NVM_RWW + i * NVMCTRL_PAGE_SIZE,
                          (uint8_t *)(&coreParams) + i * NVMCTRL_PAGE_SIZE,
                          NVMCTRL_PAGE_SIZE);
    if (sc != STATUS_OK)
      return -2;
  ...
  if (writeBytes > i * NVMCTRL_PAGE_SIZE) {
    ...
    sc = nvm_write_buffer(NVM_RWW + i * NVMCTRL_PAGE_SIZE,
                          (uint8_t *)(&coreParams) + i * NVMCTRL_PAGE_SIZE,
                          writeBytes - i * NVMCTRL_PAGE_SIZE);
    if (sc != STATUS_OK)
      return -2;
  }
...
static void upgradeCore() {
  ...
  for (i = 0; i < readBytes / NVMCTRL_PAGE_SIZE; i++) {
    // Read each page at corresponding position
    nvm_read_buffer(NVM_RWW + i * NVMCTRL_PAGE_SIZE,
                    (uint8_t *)(&coreParamsOld) + i * NVMCTRL_PAGE_SIZE,
                    NVMCTRL_PAGE_SIZE);
  }
...
static void command_data_handler(uint8_t iFace, uint8_t rxChar) {
  ...
  case STATE_CMD_IDLE:
    ...
    case 'h':
      ...
      pomelo_sprintf(iFace, "\"sn\":\"%08X%08X%08X%08X\",",
      ...
      pomelo_sprintf(iFace, "\"ecal\":[%8e,%8e,%8e],", physicsParams.ecal[0],
                     physicsParams.ecal[1], physicsParams.ecal[2]);
      if (gammaCoincidence) {
        pomelo_sprintf(iFace, "\"coincidenceCount\":%lu,",
                       gammaPulseCoincidenceN);
      }
      temp = read_temp();
      pomelo_sprintf(iFace, "\"temperature\":%.5f,", temp);
      if (gammaDaqRun) {
        pomelo_sprintf(iFace, "\"time\":%lu,",
                       rtc_count_get_count(&rtc_instance_app) -
                           gammaPulseTstart);
      } else {
        pomelo_sprintf(iFace, "\"time\":%lu,",
                       gammaPulseTstop - gammaPulseTstart);
      }
      ...
      if (gammaCoincidence) {
        ...
        for (i = 0; i < SPECTRUM_LEN - 1; i++) {
          pomelo_sprintf(iFace, "%lu,",
                         (unsigned long)gammaSpectrumCoincidence[i]);
        }
        pomelo_sprintf(
            iFace, "%lu]",
            (unsigned long)gammaSpectrumCoincidence[SPECTRUM_LEN - 1]);
      }
      pomelo_printf(iFace, "}}\n");
      break;
    case 's':
      ...
      pomelo_sprintf(iFace, "\"sn\":\"%08X%08X%08X%08X\",",
                     *(uint32_t *)0x0080A00C, *(uint32_t *)0x0080A040,
                     *(uint32_t *)0x0080A044, *(uint32_t *)0x0080A048);
      pomelo_sprintf(iFace, "\"uptime\":%lu,",
                     rtc_count_get_count(&rtc_instance_app));
    ...
    case 'c':
      ...
      pomelo_sprintf(iFace, "\"vdac\":[%f,%f],", coreParams.vDac[0],
                     coreParams.vDac[1]);
      pomelo_sprintf(iFace, "\"ecal\":[%8e,%8e,%8e],", physicsParams.ecal[0],
                     physicsParams.ecal[1], physicsParams.ecal[2]);
      pomelo_sprintf(iFace, "\"threshold\":%d,", coreParams.threshold);
      pomelo_sprintf(iFace, "\"coincidence\":%d,", coreParams.sys_coincidence);
      pomelo_sprintf(iFace,
                     "\"outputs\":{\"uart\":{\"pulse\":%d,\"fastPulse\":%d,"
                     "\"adc\":%d,\"adcTs\":%d},",
    ...
    case 'q':
      ...
      pomelo_sprintf(iFace, "\t[4-6] ecal[3] = {%f, %f, %f},\n",
                     physicsParams.ecal[0], physicsParams.ecal[1],
                     physicsParams.ecal[2]);
      ...
      pomelo_sprintf(iFace, "\t[10-12] iMeas[3] = {%f, %f, %f},\n",
                     coreParams.iMeas[0], coreParams.iMeas[1],
                     coreParams.iMeas[2]);
    ...
    case 'i':
      // Read sipm current
      pomelo_sprintf(iFace, "{\"type\":\"current\",\"payload\":{\"uA\":%f,",
                     hvload_uA());
